                ...
                if (!options.host || !options.user || !options.dumpPath || !options.database) {
                    ...
                    fs.exists((extName === '') ? options.dumpPath : dirName, function (exists) {
                        if (!exists) {
                            ...
                            ls = exec(command, {maxBuffer : 500 * 1024}, function (error, stdout, stderr) {
                                if (error !== null) {
                ...
                if (!options.user || !options.host || !options.sqlFilePath || !options.database) {
                    ...
                    fs.exists(options.sqlFilePath, function (exists) {
                        if (!exists) {
                            ...
                            if (options.password) {
                                command = util.format('mysql -f -h %s -u %s -p%s %s <%s', options.host, options.user, options.password, options.database, options.sqlFilePath);
                            } else {
                                command = util.format('mysql -f -h %s -u %s  %s <%s', options.host, options.user, options.database, options.sqlFilePath);
                            }
                            ls = exec(command, {maxBuffer : 500 * 1024}, function (error, stdout, stderr) {
                                if (error !== null) {
                                    callback(error, null, null);
